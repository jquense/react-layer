...
module.exports = (function () {
  ...
  babelHelpers.createClass(Layer, {
    render: {
      value: function render(cb, parentComponent) {
        if (!this._mountPoint) this._createMountPoint();

        var child = this._render();

        return parentComponent ? ReactDOM.unstable_renderSubtreeIntoContainer(parentComponent, child, this._mountPoint, cb) : ReactDOM.render(child, this._mountPoint, cb);
      }
    },
    unmount: {
